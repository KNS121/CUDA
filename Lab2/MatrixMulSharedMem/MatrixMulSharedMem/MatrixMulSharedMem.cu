...
__global__ void MatrixMultplyGPU_SharedMemory(int* A, int* B, int* C, int n) {
...
vector<vector<int>> MatrixMultCUDA(const vector<vector<int>>& A, const vector<vector<int>>& B, const int n) {
    ...
    MatrixMultplyGPU_SharedMemory << < kolvo_blockov, kolvo_potokov >> > (dev_A, dev_B, dev_res, n);
...
int main() {
    ...
    cout << "GPU time ( Shared mem ) : " << gpu_time.count() << " secund \n";
    //cout << "GPU result:\n";
